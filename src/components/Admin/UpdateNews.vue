...
const resetImage = () => {
  addNews.image = berita.data.image;
};
  ...
  formEl.validate((valid) => {
    if (valid) {
      if (addNews.image === "") {
        ...
        updNews(berita.data._id, addNews);
      }
    } else {
  ...
  <Teleport to="body">
    <div v-if="open" class="modal">
      <div class="bg-[var(--color-sand)] p-10 rounded-md w-[50%]">
        <div class="text-4xl text-center cw font-bold pb-5">Edit Berita</div>
        <el-form
          size="large"
          ref="addNewsref"
          label-position="top"
        ...
        >
          ...
          <el-form-item>
            <el-button @click="onSubmit(addNewsref)" type="primary"
              >Submit</el-button
            >
            <el-button @click="onReset(addNewsref), resetImage()"
              >Reset</el-button
            >
          </el-form-item>
          <div class="flex justify-end">
            <el-button
              @click="(open = false), onReset(addNewsref), resetImage()"
              type="danger"
              >Close</el-button
            >
          </div>
